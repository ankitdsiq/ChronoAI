...
def ask_user(query:str,value:str):
    prompt = PromptTemplate(
        ...
        Do not make result too long. Just keep it sort and formal
        """,
...
params = {
    ...
    'create_client':['client_name', 'client_website',  'project_name_for_this_client'],
...
def extract_parameters(state:AgentState):
    try:
        ...
        prompt = PromptTemplate(
            template="""
            Your task is to extract all the parameters mentioned {parameters} from the conversation between user and AI {conversation}
